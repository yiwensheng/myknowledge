...
name: leader
description: >-
  把一句话想法拆成 AI agent 能独立跑完的目标任务书（Goal Brief）。
  用于易知长任务：安装包瘦身、Tauri 换栈、知识库大规模整理、无人值守重构等。
  用户说「写任务书」「写 brief 给 agent」「leader」「给 agent 写目标」「让 agent 自己跑」时使用。
myknowledge:
  action: leader
  triggers:
    - 写任务书
...
> **易知内置**：位于 `skills/leader/`。
> **GUI**：工作流 →「生成目标任务书」→ 输入想法 → 运行（单轮生成，缺省决策标「猜的」）。
> **CLI**：`yws skill run leader --topic "一句话想法"`
> **查看**：`yws skill show leader` / `yws skill match 写任务书`
> 结构规格见 `references/anatomy.md`。完整多轮追问仍可在 Cursor Agent 中显式调用本 Skill。
...
```powershell
yws skill run leader --topic "把安装包再瘦 100MB，不许删测试"
```

- 参数：`topic` / `text` / `idea`（同义）
- 默认写入 `concepts/目标任务书-….md`；加环境或参数关闭保存时仅返回正文
- GUI 工作流日志展示全文，可复制给执行 Agent
...
**（易知 GUI 一键模式）** 跳过当面追问，直接按默认写「我替领导拍的板」。
